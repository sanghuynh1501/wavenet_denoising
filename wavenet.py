...
class DenoisingWavenet():
    ...
    def valid_mean_absolute_error(self, y_true, y_pred):
        return tf.math.mean(
            tf.math.abs(y_true[:, 1:-2] - y_pred[:, 1:-2]))
    ...
    def dilated_residual_block(self, data_x, condition_x, res_block_i, layer_i, dilation, stack_i):
        ...
        # Data sub-block
        data_out = keras.layers.Conv1D(filters=2 * self.config['model']['filters']['depths']['res'],
                                       kernel_size=self.config['model']['filters']['lengths']['res'],
        ...
        data_x = keras.layers.Conv1D(
            ...
            padding='same', bias=False)(data_x)
