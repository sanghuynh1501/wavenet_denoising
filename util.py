...
def l1_l2_loss(y_true, y_pred, l1_weight, l2_weight):
    ...
    if l1_weight != 0:
        loss += l1_weight * MAE(y_true, y_pred)

    if l2_weight != 0:
        loss += l2_weight * MAE(y_true, y_pred)
...
def keras_linear_to_ulaw(x, u=255.0):
    x = tf.math.sign(x) * (tf.math.log(1 + u * tf.math.abs(x)) / tf.math.log(1 + u))
    return x
...
def keras_ulaw_to_linear(x, u=255.0):
    y = tf.math.sign(x) * (1 / u) * (((1 + u) ** tf.math.abs(x)) - 1)
    return y
